...
// ApiAppStatusResponse Base serializer for use with :class:`pulpcore.app.models.Model`This ensures that all Serializers provide values for the 'pulp_href` field.The class provides a default for the ``ref_name`` attribute in theModelSerializers's ``Meta`` class. This ensures that the OpenAPI definitionsof plugins are namespaced properly.
...
type _ApiAppStatusResponse ApiAppStatusResponse
...
func (o *ApiAppStatusResponse) UnmarshalJSON(data []byte) (err error) {
	varApiAppStatusResponse := _ApiAppStatusResponse{}

	err = json.Unmarshal(data, &varApiAppStatusResponse)

	if err != nil {
		return err
	}

	*o = ApiAppStatusResponse(varApiAppStatusResponse)

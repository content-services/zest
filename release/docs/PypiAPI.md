...
> SummaryResponse PypiRead(ctx, path, pulpDomain).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
	...
	resp, r, err := apiClient.PypiAPI.PypiRead(context.Background(), path, pulpDomain).XTaskDiagnostics(xTaskDiagnostics).Fields(fields).ExcludeFields(excludeFields).Execute()
	if err != nil {

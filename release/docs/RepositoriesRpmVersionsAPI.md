...
[**RepositoriesRpmRpmVersionsScan**](RepositoriesRpmVersionsAPI.md#RepositoriesRpmRpmVersionsScan) | **Post** /{rpm_rpm_repository_version_href}scan/ | Scan for vulnerabilities
...
## RepositoriesRpmRpmVersionsScan

> AsyncOperationResponse RepositoriesRpmRpmVersionsScan(ctx, rpmRpmRepositoryVersionHref).XTaskDiagnostics(xTaskDiagnostics).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.RepositoriesRpmVersionsAPI.RepositoriesRpmRpmVersionsScan(context.Background(), rpmRpmRepositoryVersionHref).XTaskDiagnostics(xTaskDiagnostics).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `RepositoriesRpmVersionsAPI.RepositoriesRpmRpmVersionsScan``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `RepositoriesRpmRpmVersionsScan`: AsyncOperationResponse
	fmt.Fprintf(os.Stdout, "Response from `RepositoriesRpmVersionsAPI.RepositoriesRpmRpmVersionsScan`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiRepositoriesRpmRpmVersionsScanRequest struct via the builder pattern

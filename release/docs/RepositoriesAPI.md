...
------------- | ------------- | -------------
[**RepositoriesList**](RepositoriesAPI.md#RepositoriesList) | **Get** /pulp/{pulp_domain}/api/v3/repositories/ | List repositories



## RepositoriesList

> PaginatedRepositoryResponseList RepositoriesList(ctx, pulpDomain).LatestWithContent(latestWithContent).Limit(limit).Name(name).NameContains(nameContains).NameIcontains(nameIcontains).NameIexact(nameIexact).NameIn(nameIn).NameIstartswith(nameIstartswith).NameStartswith(nameStartswith).Offset(offset).Ordering(ordering).PulpHrefIn(pulpHrefIn).PulpIdIn(pulpIdIn).PulpLabelSelect(pulpLabelSelect).PulpType(pulpType).PulpTypeIn(pulpTypeIn).Q(q).Remote(remote).RetainRepoVersions(retainRepoVersions).RetainRepoVersionsGt(retainRepoVersionsGt).RetainRepoVersionsGte(retainRepoVersionsGte).RetainRepoVersionsIsnull(retainRepoVersionsIsnull).RetainRepoVersionsLt(retainRepoVersionsLt).RetainRepoVersionsLte(retainRepoVersionsLte).RetainRepoVersionsNe(retainRepoVersionsNe).RetainRepoVersionsRange(retainRepoVersionsRange).WithContent(withContent).Fields(fields).ExcludeFields(excludeFields).Execute()
...
func main() {
    ...
    pulpType := "pulpType_example" // string | Pulp type  * `rpm.rpm` - rpm.rpm (optional)
    pulpTypeIn := []string{"PulpTypeIn_example"} // []string | Multiple values may be separated by commas.  * `rpm.rpm` - rpm.rpm (optional)
 ...
 **pulpType** | **string** | Pulp type  * &#x60;rpm.rpm&#x60; - rpm.rpm | 
 **pulpTypeIn** | **[]string** | Multiple values may be separated by commas.  * &#x60;rpm.rpm&#x60; - rpm.rpm | 
 **q** | **string** |  | 

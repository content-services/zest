/*
Pulp 3 API

Fetch, Upload, Organize, and Distribute Software Packages

API version: v3
Contact: pulp-list@redhat.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zest

import (
	"encoding/json"
	"fmt"
)

// checks if the ContentViewPackageGroupResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ContentViewPackageGroupResponse{}

// ContentViewPackageGroupResponse A representation of a PackageGroup for Content View search results.
type ContentViewPackageGroupResponse struct {
	PulpHref *string `json:"pulp_href,omitempty"`
	// ID of the group
	Id string `json:"id"`
	// Name of the group
	Name string `json:"name"`
	// Description of the group
	Description string `json:"description"`
	// The list of packages in this group
	Packages interface{} `json:"packages"`
	AdditionalProperties map[string]interface{}
}

type _ContentViewPackageGroupResponse ContentViewPackageGroupResponse

// NewContentViewPackageGroupResponse instantiates a new ContentViewPackageGroupResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewContentViewPackageGroupResponse(id string, name string, description string, packages interface{}) *ContentViewPackageGroupResponse {
	this := ContentViewPackageGroupResponse{}
	this.Id = id
	this.Name = name
	this.Description = description
	this.Packages = packages
	return &this
}

// NewContentViewPackageGroupResponseWithDefaults instantiates a new ContentViewPackageGroupResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewContentViewPackageGroupResponseWithDefaults() *ContentViewPackageGroupResponse {
	this := ContentViewPackageGroupResponse{}
	return &this
}

// GetPulpHref returns the PulpHref field value if set, zero value otherwise.
func (o *ContentViewPackageGroupResponse) GetPulpHref() string {
	if o == nil || IsNil(o.PulpHref) {
		var ret string
		return ret
	}
	return *o.PulpHref
}

// GetPulpHrefOk returns a tuple with the PulpHref field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ContentViewPackageGroupResponse) GetPulpHrefOk() (*string, bool) {
	if o == nil || IsNil(o.PulpHref) {
		return nil, false
	}
	return o.PulpHref, true
}

// HasPulpHref returns a boolean if a field has been set.
func (o *ContentViewPackageGroupResponse) HasPulpHref() bool {
	if o != nil && !IsNil(o.PulpHref) {
		return true
	}

	return false
}

// SetPulpHref gets a reference to the given string and assigns it to the PulpHref field.
func (o *ContentViewPackageGroupResponse) SetPulpHref(v string) {
	o.PulpHref = &v
}

// GetId returns the Id field value
func (o *ContentViewPackageGroupResponse) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *ContentViewPackageGroupResponse) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *ContentViewPackageGroupResponse) SetId(v string) {
	o.Id = v
}

// GetName returns the Name field value
func (o *ContentViewPackageGroupResponse) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *ContentViewPackageGroupResponse) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *ContentViewPackageGroupResponse) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value
func (o *ContentViewPackageGroupResponse) GetDescription() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Description
}

// GetDescriptionOk returns a tuple with the Description field value
// and a boolean to check if the value has been set.
func (o *ContentViewPackageGroupResponse) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Description, true
}

// SetDescription sets field value
func (o *ContentViewPackageGroupResponse) SetDescription(v string) {
	o.Description = v
}

// GetPackages returns the Packages field value
// If the value is explicit nil, the zero value for interface{} will be returned
func (o *ContentViewPackageGroupResponse) GetPackages() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}

	return o.Packages
}

// GetPackagesOk returns a tuple with the Packages field value
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ContentViewPackageGroupResponse) GetPackagesOk() (*interface{}, bool) {
	if o == nil || IsNil(o.Packages) {
		return nil, false
	}
	return &o.Packages, true
}

// SetPackages sets field value
func (o *ContentViewPackageGroupResponse) SetPackages(v interface{}) {
	o.Packages = v
}

func (o ContentViewPackageGroupResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ContentViewPackageGroupResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.PulpHref) {
		toSerialize["pulp_href"] = o.PulpHref
	}
	toSerialize["id"] = o.Id
	toSerialize["name"] = o.Name
	toSerialize["description"] = o.Description
	if o.Packages != nil {
		toSerialize["packages"] = o.Packages
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *ContentViewPackageGroupResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"name",
		"description",
		"packages",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varContentViewPackageGroupResponse := _ContentViewPackageGroupResponse{}

	err = json.Unmarshal(data, &varContentViewPackageGroupResponse)

	if err != nil {
		return err
	}

	*o = ContentViewPackageGroupResponse(varContentViewPackageGroupResponse)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "pulp_href")
		delete(additionalProperties, "id")
		delete(additionalProperties, "name")
		delete(additionalProperties, "description")
		delete(additionalProperties, "packages")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableContentViewPackageGroupResponse struct {
	value *ContentViewPackageGroupResponse
	isSet bool
}

func (v NullableContentViewPackageGroupResponse) Get() *ContentViewPackageGroupResponse {
	return v.value
}

func (v *NullableContentViewPackageGroupResponse) Set(val *ContentViewPackageGroupResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableContentViewPackageGroupResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableContentViewPackageGroupResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableContentViewPackageGroupResponse(val *ContentViewPackageGroupResponse) *NullableContentViewPackageGroupResponse {
	return &NullableContentViewPackageGroupResponse{value: val, isSet: true}
}

func (v NullableContentViewPackageGroupResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableContentViewPackageGroupResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


